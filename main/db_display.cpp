...
static void reset_display()
{
    gpio_config_t reset_pin_config = {
        .pin_bit_mask = 1 << CONFIG_OLED_RST,
        .mode = GPIO_MODE_OUTPUT,
        .pull_up_en = GPIO_PULLUP_DISABLE,
        .pull_down_en = GPIO_PULLDOWN_DISABLE,
        .intr_type = GPIO_INTR_DISABLE};
    gpio_config(&reset_pin_config);
    gpio_set_level((gpio_num_t)CONFIG_OLED_RST, 0); // low
    vTaskDelay(20 / portTICK_PERIOD_MS);            // wait 20 ms
    gpio_set_level((gpio_num_t)CONFIG_OLED_RST, 1); // high
}

extern "C" void display_status(void *)
{
    ESP_LOGI(TAG, "display task started");
    SSD1306IDF display(CONFIG_OLED_ADDRESS, (gpio_num_t)CONFIG_OLED_SDA, (gpio_num_t)CONFIG_OLED_SCL);
    reset_display();
    ...
    while (1)
    {
        ...
        db_status.wifi.tx_data = 0;
        db_status.wifi.rx_data = 0;
        ...
        db_status.uart.tx_data = 0;
        db_status.uart.rx_data = 0;
